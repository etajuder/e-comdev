...
class Forum extends CI_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model(array("dataforum","datalocation","datauser","datathread", "databid"));
	...
	public function lists()
	{
		if(isset($_POST['keyword'])){
			$this->session->set_userdata("forum_search",$_POST['keyword']);
		}
		$user = $this->session->userdata("user");
		if(!in_array(@$user['level'],array("user","admin"))){
			redirect(base_url());
		}else{
			$this->load->view('content/body',array("content" => "forum/list"));
		}
	}
	public function postComment(){
		$this->load->config("setting");
		$user = $this->session->userdata("user");
		$item = $this->dataforum->getDetail($_POST['id_post']);
		...
		if ($this->form_validation->run() != FALSE){
			if($this->dataforum->saveComment($data)){
				...
				$this->dataforum->createNotif($notif);
	...
	public function add(){
		...
		if(!in_array(@$user['level'],array("user","admin"))){
			...
			$this->load->view('content/body',array("content" => "forum/add"));
		}
	}
	
	public function edit(){
		$this->load->view('content/body',array("content" => "forum/edit"));
	}
	...
	public function uploadPhoto(){
		...
		if (!$this->upload->do_upload('filePhoto')){
			...
			$insert = $this->dataforum->insertData($value);
	...
	public function addProcess(){
		...
		if ($this->form_validation->run() != FALSE){
			...
			$insertPost = $this->dataforum->insertPost($data);
			if($insertPost){
	...
	public function editProcess(){
		...
		if ($this->form_validation->run() != FALSE){
			...
			$editPost = $this->dataforum->editPost($data);
			if($editPost){
				if(count($photoUploaded)>0){
					$this->dataforum->deletePhotoLink($data['id_post']);
					foreach($photoUploaded as $key=>$val){
						$dataphoto = array(
				...
				$this->session->set_flashdata("message",array("danger","Error while saving data! Please try again."));
				redirect(base_url("forum/edit/?id=".$data['id_post']));
			}
		}else{
			$this->session->set_flashdata("message",array("danger",validation_errors()));
			redirect(base_url("forum/edit/?id=".$data['id_post']));
