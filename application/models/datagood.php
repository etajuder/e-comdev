<?php
class Datagood extends CI_Model {
	...
	public function insertbank($data){
		$query = $this->db->insert_string("tb_detail_bank",$data);
		$result = $this->db->query($query);
		return $result;
	}

	public function insertConfirm($data){
		$query = $this->db->insert_string("tb_confirmation",$data);
		$result = $this->db->query($query);
		return $result;
	}
    ...
    public function getListBank2()
    {
		$this->db->order_by("name_bank","ASC");
		$data = $this->db->get("tb_bank");
		return $data->result();
    }
    ...
    {
		$query = $this->db->update_string("tb_detail_bank",$data, "id_detailbank = '".$data['id_detailbank']."'");
		return $this->db->query($query);
    }
	...
	function getDetail($id_post){
		...
		function getdetailbankedit($id_detbank){
		$this->db->where("id_detailbank",$id_detbank);
		$query 	= $this->db->get("tb_detail_bank");
		$result = $query->result();
		return $result[0];
	}
	...
	function getListPhotoOfPost($id_post){
		...
		function getDetailBank($id){
			$query = "select * from tb_detail_bank b left join tb_bank u on b.id_bank=u.id_bank where id_user=".$id." and status='1' order by name_bank desc";
		...
		if($data->num_rows()>0){
			...
			$query = "select * from tb_detail_bank b left join tb_bank u on b.id_bank=u.id_bank where id_user=".$id." order by name_bank desc";
			$data 	= $this->db->query($query);
		if($data->num_rows()>0){
			$data = $data->result();
			return $data;
			...
			$query = "select * from tb_post  where post_type in('auction','item') order by title desc";
			$data 	= $this->db->query($query);
		if($data->num_rows()>0){
			$data = $data->result();
			return $data;
    ...
    {
		$query = "DELETE FROM `tb_detail_bank` WHERE id_detailbank=".$id."";
		$this->db->query($query);
		return $query;
    }
