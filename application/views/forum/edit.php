<?php 
	...
	$sesi = $this->dataforum->getDetail($_GET['id']);
	$photos = $this->dataforum->getListPhotoOfPost($sesi->id_post);
?>

<div class="container">
	<div class="col-sm-12">
		<h2><i class="ion-ios-cart orange"></i> Edit Your Thread</h2>
		<hr>
		<form action="<?=base_url()?>forum/editProcess" method="POST" enctype="multipart/form-data">
					...
					<select class="form-control" name="id_category" id="location" required="">
						<?php 
							...
							foreach($option as $key=>$val){
								if($sesi->id_category==$val->id_thread){
									echo "<option value='$val->id_thread' selected>$val->name_thread</option>";
								}else{
