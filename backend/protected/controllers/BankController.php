...
class BankController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Bank']))
		{
			...
			$rnd = rand(0,9999);
			$uploadedFile1=CUploadedFile::getInstance($model,'image_bank');
			$fileName1 = $rnd."-".$uploadedFile1;
            $model->image_bank = "assets/uploads/".$fileName1;
			if($model->save()){
				  if(!empty($uploadedFile1))  // check if uploaded file is set or not
                	{
                     $uploadedFile1->saveAs(Yii::app()->basePath.'/../../assets/uploads/'.$fileName1);
               		  }
               		  else
               		  {
               		  	$model->image_bank="";
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Bank']))
		{
            ...
            $model->image_bank = "assets/uploads/".$fileName1;
             if(!empty($uploadedFile1))  // check if uploaded file is set or not
                	{                		
            		$model->image_bank =  $model->image_bank;
                	}else
                	{
                		
               		  	$model->image_bank = $image_bank;
                	}
			if($model->save()){
				  if(!empty($uploadedFile1))  // check if uploaded file is set or not
                	{
